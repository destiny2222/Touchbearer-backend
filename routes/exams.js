...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (roles.includes("Parent")) {
        ...
        if (parents.length > 0) {
          ...
          const [studentClasses] = await connection.query(
            "SELECT class_id FROM students WHERE parent_id = ?",
            [parentId]
          );
          studentClasses.forEach((c) => c.class_id && classIds.add(c.class_id));

          const [newStudentClasses] = await connection.query(
            "SELECT class_id FROM new_students WHERE parent_id = ?",
            [parentId]
          );
          newStudentClasses.forEach(
            (c) => c.class_id && classIds.add(c.class_id)
          );
        }
      }
      ...
      let timeCondition = `
        (
            e.exam_date_time > NOW() 
            OR 
            (
                NOW() > e.exam_date_time 
                AND 
                NOW() < DATE_ADD(e.exam_date_time, INTERVAL 72 HOUR)
            )
        )
    `;
      let whereClauses = [
